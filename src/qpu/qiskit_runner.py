...
def load_config(config_path: Path) -> Dict[str, Any]:
    ...
    pair = raw["protocol"]["pair"]
    if not (isinstance(pair, list) and len(pair) == 2 and all(isinstance(x, int) for x in pair)):
        raise ConfigError(f"protocol.pair must be [i,j] ints, got {pair!r}")
...
# Protocol B note:
#   We build **exactly 4 circuits per context** in a fixed order (SETTINGS):
#       cos_a, sin_a, cos_b, sin_b
#   For drift-minimization we keep each context's 4 settings adjacent in the submitted list.
...
def build_protocol_b_circuits(
    ...
    """
    Build 4 circuits per context (cos_a, sin_a, cos_b, sin_b), measuring only qubit j into meas[0].
    """
    ensure_qiskit_available()
    ...
    for z_rest_int in contexts:
        ...
        for setting in SETTINGS:
            qc = QuantumCircuit(n, name=f"ctx{int(z_rest_int)}_{setting}")
            ...
            qc.metadata = {"z_rest_int": int(z_rest_int), "setting": setting, "pair": [i, j]}

            circuits.append(qc)
            tags.append(CircuitTag(z_rest_int=int(z_rest_int), setting=setting))
...
#   - NEVER split within a context: each context is 4 circuits and stays contiguous
...
def execute_qpu_batches(
    ...
    if not auto_split:
        ...
        # Ensure we don't split within a context (4 settings).
        per_ctx = len(SETTINGS)
        max_ctx_by_circuits = max(1, max_circuits_per_job // per_ctx)
        max_ctx_by_exec = max(1, max_exec_per_job // (shots * per_ctx))
        ctx_per_batch = max(1, min(max_ctx_by_circuits, max_ctx_by_exec))
        batch_size = ctx_per_batch * per_ctx

        batches = []
        s = 0
        while s < len(circuits):
            e = min(len(circuits), s + batch_size)
            # keep multiple-of-4 boundary
            e = s + ((e - s) // per_ctx) * per_ctx
            if e == s:
                e = min(len(circuits), s + per_ctx)
            batches.append((s, e))
            s = e
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Pair
    i_raw, j_raw = cfg["protocol"]["pair"]
    i, j = sorted_pair(int(i_raw), int(j_raw))
    if i < 0 or j < 0 or i >= n or j >= n:
        raise ConfigError(f"protocol.pair indices out of range for n={n}: {cfg['protocol']['pair']!r}")
    ...
    # Analytic targets
    targets = compute_targets(provider, n, i, j, contexts)
    ...
    if not args.skip_validation:
        try:
            validation_report = preflight_validate_phasors(
                n=n,
                i=i,
                j=j,
                contexts=contexts,
            ...
            if not validation_report.passed:
                raise PolicyError(
                    ...
                    f"max_err_u_a={validation_report.max_err_u_a:g}, "
                    f"max_err_u_b={validation_report.max_err_u_b:g}, "
                    f"max_err_u_kappa={validation_report.max_err_u_kappa:g} "
                    f"(tol={validation_report.tol:g}). "
    ...
    if args.validate_only:
        ...
        meta = {
            ...
            "contexts": contexts,
            "validation": dataclasses.asdict(validation_report) if validation_report is not None else None,
            "warnings": warnings,
            "errors": errors,
            "exit_code": exit_code,
    ...
    circuits, tags = build_protocol_b_circuits(n, i, j, contexts, u_gate)
    ...
    if mode == "statevector":
        if args.dry_run:
            ...
            rows, counts_payload = execute_statevector(n, i, j, contexts, targets, u_gate, bit_order=model_info.bit_order)
        ...
        if args.dry_run:
            ...
            # Aggregate into rows per context
            by_ctx: Dict[int, Dict[str, float]] = {int(z): {} for z in contexts}
            for entry in per_circuit_counts:
                ...
                by_ctx[z][setting] = float(m)

            for z_rest_int in contexts:
                z_rest_int = int(z_rest_int)
                obs = by_ctx.get(z_rest_int, {})
                missing = [s for s in SETTINGS if s not in obs]
                if missing:
                    msg = (
                        f"Missing settings {missing} for z_rest_int={z_rest_int}. "
                        "Protocol B requires all 4 settings; marking row invalid (NaN) to avoid silent corruption."
                    )
                    errors.append(msg)
                    dropped_contexts.append({'z_rest_int': int(z_rest_int), 'missing_settings': list(missing)})
