...
# Schedule modes (CLI-only; do not add YAML keys)
SCHEDULE_MODES: Tuple[str, str] = ("normal", "blocked_by_setting")


def parse_blocked_order(order_csv: str) -> List[str]:
    """Parse a comma-separated blocked execution order.

    Contract
    --------
    - Input is a comma-separated list of settings.
    - Must be a permutation of SETTINGS.
    - Raises ConfigError on invalid input.
    """
    if order_csv is None:
        raise ConfigError("blocked order must be a non-empty comma-separated list")

    parts = [p.strip() for p in str(order_csv).split(",") if p.strip()]
    if len(parts) != len(SETTINGS):
        raise ConfigError(
            f"--blocked-order must contain exactly {len(SETTINGS)} comma-separated settings; got {parts!r}"
        )
    if len(set(parts)) != len(parts):
        raise ConfigError(f"--blocked-order contains duplicates: {parts!r}")

    allowed = set(SETTINGS)
    unknown = [p for p in parts if p not in allowed]
    if unknown:
        raise ConfigError(
            f"--blocked-order contains unknown settings {unknown!r}. Allowed: {list(SETTINGS)!r}"
        )
    return parts


def counts_key_from_tag(tag: "CircuitTag") -> Tuple[int, int, int, str]:
    """Canonical key for a circuit/tag: (i,j,z_rest_int,setting)."""
    return (int(tag.i), int(tag.j), int(tag.z_rest_int), str(tag.setting))


def counts_key_from_entry(entry: Dict[str, Any]) -> Tuple[int, int, int, str]:
    """Canonical key for a per-circuit counts entry: (i,j,z_rest_int,setting)."""
    ...
    return (int(pair_list[0]), int(pair_list[1]), int(entry["z_rest_int"]), str(entry["setting"]))


def reorder_counts_like_tags(
    per_circuit_counts: List[Dict[str, Any]],
    tags: List["CircuitTag"],
) -> Tuple[List[Dict[str, Any]], List[Tuple[int, int, int, str]], List[Tuple[int, int, int, str]]]:
    """Reorder a per-circuit list to match the canonical tag ordering.

    This is used by blocked scheduling, where circuits are executed in 4 separate jobs
    but downstream code expects a stable per-circuit ordering for reproducibility.

    Returns:
      (ordered_counts, missing_keys, duplicate_keys)
    """
    index: Dict[Tuple[int, int, int, str], Dict[str, Any]] = {}
    duplicates: List[Tuple[int, int, int, str]] = []
    for ent in per_circuit_counts:
        k = counts_key_from_entry(ent)
        if k in index:
            duplicates.append(k)
        index[k] = ent
    ...
    for tag in tags:
        k = counts_key_from_tag(tag)
        if k not in index:
            missing.append(k)
        else:
            ordered.append(index[k])
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--schedule-mode",
        type=str,
        choices=list(SCHEDULE_MODES),
        default="normal",
        help=(
            "Execution scheduling policy. "
            "normal: submit 4 settings consecutively per (pair,context). "
            "blocked_by_setting: submit 4 separate jobs (one per setting) over all contexts×pairs, "
            "then merge in-memory (Appendix drift ablation)."
        ),
    )
    ap.add_argument(
        "--blocked-order",
        ...
        help=(
            "blocked_by_setting: comma-separated order of settings to submit as separate jobs. "
            "Example: 'cos_a,sin_a,cos_b,sin_b'."
        ),
    )
    ...
    # Schedule mode is a CLI-only toggle (YAML schema must stay stable).
    # We parse/validate here so any input mistakes fail fast *before* expensive
    # transpilation or QPU submission.
    schedule_mode_effective: str = str(args.schedule_mode)
    blocked_order_effective: Optional[List[str]] = None
    if schedule_mode_effective == "blocked_by_setting":
        if mode not in ("qpu", "fake_backend"):
            warnings.append(
                "--schedule-mode blocked_by_setting is only supported in qpu/fake_backend; "
                "ignoring and using normal schedule"
            )
            schedule_mode_effective = "normal"
        else:
            blocked_order_effective = parse_blocked_order(args.blocked_order)

    # Optional run_meta.json augmentation (allowed to add new fields for provenance).
    ...
    if mode == "statevector":
        ...
        if args.dry_run:
            ...
            if mode == "fake_backend":
                if schedule_mode_effective == "normal":
                ...
                elif schedule_mode_effective == "blocked_by_setting":
                    if blocked_order_effective is None:
                        raise ExecutionError("Internal error: blocked_order_effective is None in blocked_by_setting mode")

                    per_circuit_counts_all: List[Dict[str, Any]] = []
                    jobs_meta_all: List[Dict[str, Any]] = []

                    for phase_idx, setting in enumerate(blocked_order_effective):
                        subset_circs = [c for (c, tag) in zip(transpiled, tags) if tag.setting == setting]
                        subset_tags = [tag for tag in tags if tag.setting == setting]

                        expected = int(len(contexts) * len(pairs))
                        if len(subset_circs) != expected:
                            raise ExecutionError(
                                "Internal error: unexpected circuit subset size in blocked_by_setting. "
                                f"setting={setting!r}, len(subset_circs)={len(subset_circs)}, expected={expected}. "
                                "This suggests the Protocol B circuit ordering/tagging contract was violated."
                            )

                        subset_counts = execute_fake_backend(
                            circuits=subset_circs,
                            tags=subset_tags,
                            snapshot=snapshot,
                            shots=shots,
                            enable_gate_noise=bool(args.enable_gate_noise),
                        ...
                        per_circuit_counts_all.extend(subset_counts)
                        jobs_meta_all.append(
                            {
                                "schedule_mode": "blocked_by_setting",
                                "blocked_setting": str(setting),
                                "blocked_phase": int(phase_idx),
                                "num_circuits": int(len(subset_circs)),
                                "shots": int(shots),
                                "mode": "fake_backend",
                    ...
                    ordered_counts, missing_keys, duplicate_keys = reorder_counts_like_tags(per_circuit_counts_all, tags)
                    if duplicate_keys:
                        errors.append(
                            "blocked_by_setting produced duplicate per-circuit entries (BUG). "
                            f"duplicates(example up to 5)={duplicate_keys[:5]!r}"
                        )
                        exit_code = max(exit_code, 2)
                    if missing_keys:
                        errors.append(
                            "blocked_by_setting produced missing per-circuit entries (BUG or partial execution). "
                            f"missing_count={len(missing_keys)} example={missing_keys[0]!r}"
                        )
                        exit_code = max(exit_code, 2)

                    per_circuit_counts = ordered_counts if ordered_counts else per_circuit_counts_all
                    jobs_meta = jobs_meta_all
                    schedule_summary = {
                        "mode": "blocked_by_setting",
                        "order": list(blocked_order_effective),
                        "shots_per_circuit": int(shots),
                        "num_phases": int(len(blocked_order_effective)),
                    }
                else:
                    raise ConfigError(f"Unknown --schedule-mode: {schedule_mode_effective!r}")
                ...
                if schedule_mode_effective == "normal":
                    ...
                    # For blocked-by-setting we keep (all pairs) together within each context.
                    circuits_per_context_block_setting = int(len(pairs))
                    ...
                    for phase_idx, setting in enumerate(blocked_order_effective):
                        ...
                        subset_counts, phase_jobs_meta = execute_qpu_batches(
                            circuits=subset_circs,
                            tags=subset_tags,
                            circuits_per_context_block=circuits_per_context_block_setting,
                            cfg=cfg,
                            snapshot=snapshot,
                            selected_backend_name=selected_backend_name,
                            shots=shots,
                        ...
                        # Annotate jobs meta so Appendix drift ablations can audit per-phase jobs.
                        for jm in phase_jobs_meta:
                            jm["schedule_mode"] = "blocked_by_setting"
                            jm["blocked_setting"] = str(setting)
                            jm["blocked_phase"] = int(phase_idx)

                        per_circuit_counts_all.extend(subset_counts)
                        jobs_meta_all.extend(phase_jobs_meta)
                    ...
                    if missing_keys:
                        errors.append(
                            "blocked_by_setting produced missing per-circuit entries (BUG or partial job results). "
                            f"missing_count={len(missing_keys)} example={missing_keys[0]!r}"
                        )
                        exit_code = max(exit_code, 2)
                    ...
                    if missing:
                        if schedule_mode_effective == "blocked_by_setting":
                            msg = (
                                ...
                                "under --schedule-mode blocked_by_setting. "
                                "This should not happen in a successful blocked run and indicates a scheduling bug "
                                "or partial job results. Protocol B requires all 4 settings; marking row invalid (NaN) "
    ...
    # Optional schedule provenance (allowed: run_meta.json is backward compatible with new fields).
    if schedule_summary is not None:
        meta["schedule"] = schedule_summary
